...
7. 구글 드라이브에서 원고가 있는 폴더를 우클릭 → "공유" → 6번의 이메일 주소를 추가 → 권한은 **"편집자"**로 지정
   (학습한 스타일 가이드/진행 상태를 이 폴더 안에 파일로 저장해두고 재배포 후에도 이어서 쓰기 때문에, "뷰어"로만
   공유하면 저장이 안 되고 매번 처음부터 다시 학습하게 됩니다)
